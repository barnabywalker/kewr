...
test_that("raises error for unimplemented keyword", {
  ...
  expect_error(search_powo(query, .wait=0.15),
...
test_that("accepted filter only returns accepted names", {
  ...
  results <- search_powo(query, filters, .wait=0.15)
...
test_that("tidy lookup results returns tibble", {
  results <- lookup_powo("30001404-2", .wait=0.15)
  tidied <- tidy(results)
...
test_that("cursor returns next page of results", {
  ...
  page1 <- search_powo(query, .wait=0.15)
  page2 <- search_powo(query, cursor=page1$cursor, .wait=0.15)
